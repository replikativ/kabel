...
(deftest authorize-gate-unification
  ...
  (testing "each layer's legacy shape keeps its own argument order"
    ;; Compatibility is per-layer precisely because the two orders differ.
    (let [pubsub-seen (atom nil)
          diss-seen (atom nil)
          pg (authz/gate {:authorize-fn (fn [p t] (reset! pubsub-seen [p t]) true)}
                         {:op :subscribe :legacy-keys [:authorize-fn]
                          :legacy-adapter authz/pubsub-legacy})
          dg (authz/gate {:authorize-fn (fn [t o] (reset! diss-seen [t o]) true)}
                         {:op :publish :legacy-keys [:authorize-fn]
                          :legacy-adapter authz/dissemination-legacy})]
      (pg {:principal :alice :topic :t})
      (dg {:principal :alice :topic :t})
      (is (= [:alice :t] @pubsub-seen))
      (is (= [:t :alice] @diss-seen) "the dissemination order is reversed")))
